-- =============================================================================
-- SGTD — Migración 013
-- Archivo: 013_fix_enum_estado_tarea.sql
--
-- Causa raíz del error 42804:
--   La columna tarea.estado es tipo ENUM (estado_tarea), no TEXT.
--   InsForge creó el tipo automáticamente. Las RPCs anteriores asignan
--   valores TEXT directamente → Postgres rechaza sin cast explícito.
--
-- Causa raíz del error PGRST203 (overloading):
--   sgtd_reprogramar_tarea_con_log existe en dos versiones con firmas
--   distintas (p_nueva_fecha DATE vs TEXT). PostgREST no puede elegir.
--
-- Solución:
--   1. DROP de todas las versiones conflictivas de las tres RPCs.
--   2. Recrear con cast ::public.estado_tarea en cada asignación.
--   3. Una sola firma por función.
--
-- IMPORTANTE: Aplicar en el SQL Editor de InsForge como único bloque.
-- =============================================================================

BEGIN;

-- =============================================================================
-- Limpiar versiones anteriores (todas las sobrecargas)
-- =============================================================================

DROP FUNCTION IF EXISTS public.sgtd_cambiar_estado_tarea(UUID, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.sgtd_mover_tarea_columna(UUID, TEXT, UUID, TEXT);
DROP FUNCTION IF EXISTS public.sgtd_reprogramar_tarea_con_log(UUID, UUID, TEXT, TEXT, TEXT);
DROP FUNCTION IF EXISTS public.sgtd_reprogramar_tarea_con_log(UUID, UUID, DATE, TEXT, TEXT);

-- =============================================================================
-- 1. sgtd_cambiar_estado_tarea
--    Cast correcto: p_nuevo_estado::public.estado_tarea
-- =============================================================================

CREATE FUNCTION public.sgtd_cambiar_estado_tarea(
  p_tarea_id      UUID,
  p_nuevo_estado  TEXT,
...
DECLARE
  ...
  v_tipo_log      TEXT;
BEGIN
  -- Leer estado actual (cast ENUM → TEXT para comparar como texto)
  SELECT estado::TEXT
...
--    Alias usado por el Tablero y useMiSemana para iniciar/bloquear.
--    Delega en sgtd_cambiar_estado_tarea.
-- =============================================================================

CREATE FUNCTION public.sgtd_mover_tarea_columna(
  p_tarea_id      UUID,
  p_nuevo_estado  TEXT,
  p_usuario_id    UUID,
  p_justificacion TEXT DEFAULT NULL
...
--    Una sola firma: p_nueva_fecha como TEXT (YYYY-MM-DD).
--    Cast TEXT → DATE y TEXT → ENUM internamente.
-- =============================================================================

CREATE FUNCTION public.sgtd_reprogramar_tarea_con_log(
  p_tarea_id      UUID,
  p_usuario_id    UUID,
  p_nueva_fecha   TEXT,
...
BEGIN
  ...
  -- Calcular semana ISO YYYYWW desde la nueva fecha
  ...
  -- Cast TEXT → DATE y TEXT → ENUM
  UPDATE public.tarea
  SET
    fecha_planificada  = p_nueva_fecha::DATE,
    semana_planificada = v_nueva_semana,
    estado             = v_estado_resultante::public.estado_tarea,
    updated_at         = now()
  WHERE id = p_tarea_id;
...
--   );
-- → Exactamente 3 filas, una por función.
--
-- Test rápido (reemplazar con UUIDs reales):
-- SELECT public.sgtd_cambiar_estado_tarea(
--   '<uuid-tarea-en-estado-atrasada>',
--   'en_progreso',
--   NULL
-- );
-- =============================================================================
