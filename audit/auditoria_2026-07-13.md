...
Resultado tras este lote: `hooks/` 0% → 9.46% statements; global 27.45% → 31.54%. Sigue habiendo ~27 archivos de `hooks/` en 0% (la mayoría hooks `useXxxPage`, de mayor costo de setup por depender de TanStack Query + múltiples módulos de `api/`) — este hallazgo permanece abierto como backlog, no se marca resuelto.
