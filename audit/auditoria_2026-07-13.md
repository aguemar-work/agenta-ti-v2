...
**✅ Capa `api/` completa — las 21 unidades de test ahora cubren los 21 archivos de `api/*.ts`.** Resultado acumulado de la sesión: `api/` 3.36% → 70.48% statements; global 9.21% → 27.45%. **`hooks/` sigue en 0% (35 archivos) — ese es el trabajo pendiente restante de A2**, este hallazgo permanece abierto como backlog, no se marca resuelto.
