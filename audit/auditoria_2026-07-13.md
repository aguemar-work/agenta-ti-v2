...
Resultado acumulado de la sesión: capa `api/` completa 3.36% → 51.17% statements (`semana.ts` 74.5%, `workspace.ts` 84.4%, `invitacion.ts` 91.4%, `plataforma.ts` 85.7%, `ordenTrabajo.ts` 54.9%, `organizacion.ts` 96%, y 7 módulos más entre 51%-100%); global 9.21% → 22.32%. Sigue habiendo ~6 archivos en `api/` (los más grandes: `hoyColumnas.ts`, `objetivosMetricas.ts`, `audit.ts`, `planificacion.ts`) y ~35 en `hooks/` en 0% — este hallazgo permanece abierto como backlog, no se marca resuelto.
